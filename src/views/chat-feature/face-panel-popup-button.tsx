...
function FacePanelPopup() {
  ...
  useEffect(() => {
    const firstFacePackage = firstItemOrDefault(facePackages);
    if (firstFacePackage) {
      naviagtor(firstFacePackage.id);
    }
  }, [facePackages, naviagtor]);
...
function FaceHeader({ facesState, displayFaceId }: FaceHeaderProps) {
  ...
  return (
    <div className="FacePanelPopup__faceHeader">
      {displayFace ? (
        <img
